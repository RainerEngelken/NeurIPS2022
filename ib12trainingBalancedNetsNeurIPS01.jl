...
function generateInputOutputOUDelayAnalyticalIncremental(B, T, seedInput, S, Iext, sigma, tauS, dt, delay)
    ...
    t = 1:T # time
    s = zeros(Float32, T + delay, S, B) # input
    target = zeros(Float32, T, S, B) # target output
    rng = Xoroshiro128Star(seedInput)
    Random.seed!(rng, seedInput)
    rtargscalar = ones(Float32, T)
    rtargscalar1P = rtargscalar
    expfactorOU = exp(-dt * inv(tauS))
    stdfactorOU = Float32(sigma * sqrt.((1 .- exp.(-2dt * inv(tauS))) ./ 2tauS))
    for bi = 1:B #put in right format for batch-gradient descent
        for si = 1:S
            sigmasqrtdt = Float32(sigma * sqrt(dt))
            noisehere = stdfactorOU * randn(rng, Float32)
            for i = 2:(T+delay)
                noisehere = randn(rng, Float32)
                if tauS == 0.0
                    s[i, si, bi] = sigma * noisehere
                else
    ...
    return s[1+delay:end, :, :] .+ Float32(Iext), (s[1:end-delay, :, :] .+ Float32(Iext))
end




function trainCopy(N, Nepochs, B, S, T, seedIC, seedInput, seedNet, lrIn, beta1, beta2, IC, startRandom, plotStuff, tauS, g, gbar, Iext, I1, dt, delay, sigmaNoise, phi, wsStd, wsMean, wrStd, wrMean,ActivityReg, resetDiagonal,subDir)
    @show N, Nepochs, B, S, T, seedIC, seedInput, seedNet, lrIn, beta1, beta2, IC, startRandom, plotStuff, tauS, g, gbar, Iext, I1, dt, delay, sigmaNoise, phi, wsStd, wsMean, wrStd, wrMean,ActivityReg
    verbose = false
    phid(x) = ForwardDiff.derivative(phi, x)
    R = S # number of readouts
    onlyfinaltesterror = false
    cb_interval = 1000
    diagscaling = 1.0f0 # scaling of diagonal of recurrent network
    println("T:", T)
    lr = Float32(lrIn)# lrIn.0001f0 #learning rate
    nMeasure = 5
    tau = 1# just for plotting (in ms)
    ParaString = "N=$N.Nepochs=$Nepochs.B=$B.S=$S.T=$T.seedIC=$seedIC.seedInput=$seedInput.seedNet=$seedNet.lrIn=$lrIn.beta1=$beta1.beta2=$beta2.IC=$IC.tauS=$tauS.g=$g.gbar=$gbar.Iext=$Iext.I1=$I1.dt=$dt.delay=$delay.sigma=$sigmaNoise.ActivityReg=$ActivityReg.resetDiagonal=$resetDiagonal"
    if isfile("data-cube-trainedPara/" * ParaString * "testError.dat")
        testError = readdlm("data-cube-trainedPara/" * ParaString * "testError.dat")
        return testError[:]
    end
    s, rtarg = generateInputOutputOUDelayAnalyticalIncremental(B, T, seedInput, S, Iext, I1, tauS, dt, delay)
    rmul!(rtarg, -inv(gbar))
    if startRandom
        Random.seed!(seedNet)
       ...
       elseif IC == 2 #MIXED EI network, parameters from Van Vreeswijk, Sompolinsky 1996
        ...
        J = Float32.([JEE/(N/2) .+ g*randn(div(N,2),div(N,2)) / Float32(sqrt(N)) JEI/(N/2) .+ g*randn(div(N,2),div(N,2)) / Float32(sqrt(N));JIE/(N/2) .+ g*randn(div(N,2),div(N,2)) / Float32(sqrt(N)) JII/(N/2) .+ g*randn(div(N,2),div(N,2)) / Float32(sqrt(N))])
            figure(141);imshow(J);colorbar();figure()
            #J = Float32(g) * randn(Float32, N, N) / Float32(sqrt(N)) .+ Float32(gbar / N)
            resetDiagonal && (J .-= Diagonal(J))
        else
    ...
    function calcloss(s, rtarg)
        ...
        for ti = 1:T#loss # idea of min(50 + EI, NT) is curriculum learning: make problem successivly more difficult
            x += dt * (-x + J * phi.(x) .+ b + ws * s[ti, :, :] / S) + sigmaNoise * randn(Float32, N, B) * sqrt(dt)
            if ti >= div(T, 5)
                r = wr * phi.(x) / N
                loss += sum(abs2,r - rtarg[ti, :, :]) + ActivityReg*sum(abs2,x) #/NT#loss # loss is MSE normalized by batch size
            end
            #@show typeof(loss)
        end
    ...
    function calclossend(x0)
        ...
        for ti = 1:T#loss # idea of min(50 + EI, NT) is curriculum learning: make problem successivly more difficult
            ...
            r = wr * phi.(x)
            if ti == T
                loss += sum((r - rtarg[ti, :, :]) .^ 2)  #/NT#loss # loss is MSE normalized by batch size
            end
        end
        return loss / B
    end
...
mod(ei, 10) == 0 && print(ei, "\r")
        ...
        EI = ei
        Random.seed!(ei)
        s, rtarg = generateInputOutputOUDelayAnalyticalIncremental(B, T, seedInput + ei, S, Iext, I1, tauS, dt, delay)#(B, T, seedInput, S, sigma,tauS,dt)
        ...
        if (!onlyfinaltesterror && mod(ei, cb_interval) == 1) || ei == Nepochs
            ...
            s, rtarg = generateInputOutputOUDelayAnalyticalIncremental(B, T, seedInput + Nepochs + 1, S, Iext, I1, tauS, dt, delay)#(B, T, seedInput, S, sigma,tauS,dt)
            resetDiagonal && (J .-= Diagonal(J))
...
subplot(236)
id1wr = sortperm(ws[:,1]);id=id1wr;as=ifelse.(ws[:,1].>median(ws[:,1]),1,2)
...
@time out = kmeans(J,2;maxiter=200)
as = assignments(out);id = sortperm(as)
...
imshow(Jhere,cmap=ColorMap("RdBu"),vmin=-ve,vmax=ve,origin="lower");colorbar(fraction=0.046, pad=0.04)
suptitle(ParaString)
tight_layout()
draw()
        end
